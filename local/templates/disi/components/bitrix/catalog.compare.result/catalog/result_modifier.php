...
foreach ($arResult['ITEMS'] as $item) {
    $itemSections[] = $item['IBLOCK_SECTION_ID'];
}
$arResult['SECTIONS'] = [];
...
if (count($itemSections) > 0) {
    $dbSections = CIBlockSection::GetList(
        ["sort" => "ASC"],
        ['IBLOCK_ID'=> $arParams['IBLOCK_ID'], "ID" => $itemSections],
        false,
        ["NAME", "ID"]
    );

    while ($s = $dbSections->fetch()) {
        $arResult['SECTIONS'][$s['ID']] = $s['NAME'];
    }
}

foreach ($arResult['ITEMS'] as $i => $item) {
    $props = \Meven\Helper\Helper::getRussvetProperties($item['ID']);
    foreach ($props as $prop) {
        $arResult['SHOW_PROPERTIES'][$prop['NAME']] = [
            'NAME' => $prop['NAME'],
        ];
        ...
        $arResult['ITEMS'][$i]['PROPERTIES'][$prop['NAME']] = $value;
        $arResult['ITEMS'][$i]['DISPLAY_PROPERTIES'][$prop['NAME']] = $value;
    }
}