...
class CSeoMeta extends Bitrix\Iblock\Template\Functions\FunctionBase {
    ...
    public function AddAdditionalFilterResults($FilterAdditionalResult, $kombox = false) {
        ...
        if($kombox != 'Y') {
            foreach($FilterAdditionalResult as $key => $value) {
                if(mb_stripos($key, 'PROPERTY_') === 0 || mb_stripos($key, '=PROPERTY_') === 0 || mb_stripos($key, '>=PROPERTY_') === 0 || mb_stripos($key, '<=PROPERTY_') === 0 || mb_stripos($key, '><PROPERTY_') === 0) {
                    $property_key = $key;
                    if(is_array($value)) {
                        $Xvalue = $value;
                    }
                    ...
                    if(mb_stripos($key, 'PROPERTY_') === 0)
                        $key = str_replace('PROPERTY_', '', $key);
                    else if(mb_stripos($key, '=PROPERTY_') === 0)
                        $key = str_replace('=PROPERTY_', '', $key);
                    else if(mb_stripos($key, '>=PROPERTY_') === 0)
                        $key = str_replace('>=PROPERTY_', '', $key);
                    else if(mb_stripos($key, '<=PROPERTY_') === 0)
    ...
    function CacheConstantCheck() {
        ...
        else if(COption::GetOptionString(self::MODULE_ID, "MANAGED_CACHE_ON",'N', SITE_ID) == "N") {
        ...
        if($type === 0) {
            if($idCondition == '' && isset($FilterResult['ITEMS'][$idSection]['VALUES']) && is_array($FilterResult['ITEMS'][$idSection]['VALUES'])) {
                ...
                if(isset($FilterResult['ITEMS'][$idSection]['VALUES']) && is_array($FilterResult['ITEMS'][$idSection]['VALUES']))
                    foreach($FilterResult['ITEMS'][$idSection]['VALUES'] as $key => $param) {
                        if((is_int($idCondition) && is_numeric($key) && $key == $idCondition) || strcmp($param['VALUE'], $idCondition) == 0 || strcmp($param['LIST_VALUE'], $idCondition) == 0 || /*(isset($param['FACET_VALUE']) && strcmp($param['FACET_VALUE'], $idCondition) == 0) ||*/
    ...
    private function CheckElementsContain($idSection, $idCondition, $type = 0) {
        ...
        if($type == 0) {
            if($idCondition == '') {
                ...
                foreach($FilterResult['ITEMS'][$idSection]['VALUES'] as $key => $param) {
                    if(mb_stripos($param['VALUE'], $idCondition) !== false && isset($param['CHECKED']) && $param['CHECKED'] == 1) {
                        self::$CheckerRule[$idSection][$key] = 0;
                        $return = 1;
                    }
    ...
    private function CheckElementsNotCont($idSection, $idCondition, $type = 0) {
        ...
        if($type == 0) {
            if($idCondition == '') {
                ...
                if(mb_stripos($CheckedElements, $idCondition) !== false || $CheckedElements == '')
                    $return = 0;
                else
                    $return = 1;
            }
