...
$MESS[$module_id."_FILTER_SEF_NOTE"] = "В поле можно указать маску ссылки битрикс, отличной от стандартной, формата /filter/#FILTER_PARAMS#/apply<br><br><b>Например:</b><br>/f/#FILTER_PARAMS#/a<br>или<br>/filter/#FILTER_PARAMS#";
...
$MESS[$module_id.'_GENERATE_ALL_CONDITIONS'] = "Перегенерировать все ЧПУ ссылки при построении карты сайта";
$MESS[$module_id.'_GENERATE_ALL_CONDITIONS_NOTE'] = "При установленном чекбоксе, при генерации карты сайта, все ЧПУ ссылки будут перегенерированны в каждом из условий.<br>При установленном чекбоксе время построения карты сайта может значительно увеличиться.<br><br><b>Рекомендация:</b><br>При генерации карты сайта рекомендуется снимать данный чекбокс, для ускорения построения карты сайта.";

$MESS[$module_id.'_SITEMAP_COUNT_LINKS_FOR_OPERATION'] = "Количество ссылок обрабатываемых за один шаг, при генерации карты сайта";
$MESS[$module_id.'_SITEMAP_COUNT_LINKS_FOR_OPERATION_NOTE'] = "При установленном чекбоксе, будет браться установленное количество ссылок для обработки.<br>Если во время генерации карты сайта возникает ошибка, завершения скрипта из-за истечения времени ожидания (TIME OUT),<br>рекомендуется уменьшить указанное значение.<br><br><b>Рекомендация:</b><br>Для увеличения скорости построения карты сайта можно увеличить это значение,<br>но скрипт может завершить свое выполнение по истечению времени (TIME OUT),<br> по этому данное значение подбирается опытным путем.<br><br>(Значение по умолчанию: 10000)";
