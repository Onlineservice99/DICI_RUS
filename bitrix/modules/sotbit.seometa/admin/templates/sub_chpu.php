...
while($arRes = $rsData->NavNext(true, ""))
{
    ...
    if($arRes['T']=='P')
    {
        ...
        foreach($props as $code => $value) {
            ...
            $glue = ', ';
            if(!$name) {
                $priceItem = CIBlockPriceTools::GetCatalogPrices($arRes['iblock_id'], array($code))[$code];
    ...
    if(is_array($arSites = unserialize($condition['SITES'])) && count($arSites) == 1) {
        ...
        $row->AddViewField("REAL_URL", '<a href=\''. $protocol . $siteUrl . $arRes['REAL_URL'] .'\'>'.$arRes['REAL_URL'].'</a>');
        $row->AddViewField("NEW_URL", '<a href=\''. $protocol.  $siteUrl . $arRes['NEW_URL'] .'\'>'.$arRes['NEW_URL'].'</a>');
    }
