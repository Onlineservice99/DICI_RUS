...
if($ID > 0)
{
    ...
    if($meta && is_array($meta) && class_exists('\Bitrix\Main\Text\Emoji')) {
        foreach ($meta as $key => $value) {
            $meta[$key] = \Bitrix\Main\Text\Emoji::decode($value);
        }
    }
}
...
if($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid())
{
    if($meta && class_exists('\Bitrix\Main\Text\Emoji')) {
        foreach ($meta as $key => $value) {
            $meta[$key] = \Bitrix\Main\Text\Emoji::encode($value);
        }
    }
...
<form method="POST" action="<?=$APPLICATION->GetCurPage()?>" enctype="multipart/form-data" name="post_form">
    ...
    <tr>
        ...
        <td>
			<textarea name="NAME_TEMPLATE" id="NAME_TEMPLATE" cols="55" rows="1" style="width: 90%;"><?=$condition['NAME_TEMPLATE']?></textarea>
