...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()) {
    ...
    if ($arFields) {
        foreach ($arFields as $key => $item) {
            if (stripos($key,
                    'ELEMENT_') !== false && stripos($key,
                    '_TYPE') === false) {
                if (class_exists('\Bitrix\Main\Text\Emoji')) {
                    $item = \Bitrix\Main\Text\Emoji::encode($item);
                }
    ...
    if ($ID > 0) {
        $result = SeometaNotConfiguredPagesTable::update($ID,
            $arFields);
        if (!$result->isSuccess()) {
...
<tr class="adm-detail-valign-top">
    ...
    <td width="50%">
        ...
        CFileMan::AddHTMLEditorFrame(
            "META_ELEMENT_TOP_DESC",
            (isset($arFields['META_ELEMENT_TOP_DESC']) && !empty($arFields['META_ELEMENT_TOP_DESC'])) ? $arFields['META_ELEMENT_TOP_DESC'] : '',
...
<tr class="adm-detail-valign-top">
    ...
    <td width="50%">
        ...
        CFileMan::AddHTMLEditorFrame(
            "META_ELEMENT_BOTTOM_DESC",
            (isset($arFields['META_ELEMENT_BOTTOM_DESC']) && !empty($arFields['META_ELEMENT_BOTTOM_DESC'])) ? $arFields['META_ELEMENT_BOTTOM_DESC'] : '',
...
<tr class="adm-detail-valign-top">
    ...
    <td width="50%">
        ...
        CFileMan::AddHTMLEditorFrame(
            "META_ELEMENT_ADD_DESC",
            (isset($arFields['META_ELEMENT_ADD_DESC']) && !empty($arFields['META_ELEMENT_ADD_DESC'])) ? $arFields['META_ELEMENT_ADD_DESC'] : '',
...
$arButtonsParams = array(//    "back_url" => $backUrl,
);
